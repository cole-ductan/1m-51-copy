# 51 Locations — All in One

**35 confirmed, 5 probable, 3 unconfirmed = 43 with answers, 8 unsolved/denied** | *Feb 20, 2026*

**Feedback?** [discord.gg/KCtjNMJeYv](https://discord.gg/KCtjNMJeYv)

| # | Category | Location | Puzzle | Confidence |
|---|----------|----------|--------|------------|
| 1 | Playlist | MAPUTO | First letters of "Missing A Playlist? Use This One:" | Confirmed |
| 2 | Screen 1 + Bank | **BENIN CITY?** | "Mark BENI off" from Benioff. Backwards-J = recade (Benin symbol). | Unconfirmed |
| 3 | Screen 2 + Bank | **SEOUL** | RAINCOAT → anagram → CROATIAN. Croatian calendar month names + circled dates (DD/MM) → SEOUL. Omega ⬤. | Confirmed |
| 4 | Screen 3 + Bank | CAIRO | Candyland rebus + Feastables shirt. | Confirmed |
| 5 | Screen 4 + Bank | **UPPER HUTT** | Cosine pattern removes 0s/1s → T9 keypad → UPPER HUTT NEW ZEALAND. Omega ⬤ in list. | Probable |
| 6 | Screen 5 + Bank | **ANKARA** | ARABESQUE/ARACHNID → ARA. TANK/BANK → ANK. ARA+ANK = ANKARA. | Confirmed |
| 7 | Screen 6 + Bank | **TBILISI?** | Bird cipher → DINAMO. Coffee cup → Georgia outline. DINAMO + Georgia → FC Dinamo Tbilisi. | Probable |
| 8 | Screen 7 + Bank | **TOAD SUCK, AR** | 3-part decode: "towed"→TOAD, IPESIT→STUCK−T→SUCK, "arrrr"+Kansas→ARKANSAS. Omega ⬤. | Confirmed |
| 9 | Screen 8 + Bank | TIJUANA | Caesar busts → Slack ringtone → "THE BEST BANK" → Caesar cipher → "SALAD ORIGIN" → Tijuana. | Confirmed |
| 10 | Screen 9 + Bank | ??? | All images start with SW (confirmed by Lone Shark). Genuinely unsolved. | Unsolved |
| 11 | Screen 10 + Bank | ??? | 10⁵ + Sudoku. Case #271829 = e×10⁵. Collaboration-gated. | Unsolved |
| 12 | Screen 11 + Bank | ??? | BeastBot **denied** Lincoln. "Take it one more step." | Denied |
| 13 | Screen 12 + Bank | **MOSCOW** | Red + Cube = Red Square. BeastBot: "83.33% atypical." Omega ⬤. | Confirmed |
| 14 | Commercial | ACCRA | Vault door Scrabble tiles → "last Scrabble Championship" site | Confirmed |
| 15 | Commercial | KABUL | Morse code light flashes backwards | Confirmed |
| 16 | Commercial | ARLES | Laser pattern "LASER MIX UP" anagram | Confirmed |
| 17 | Commercial | KUPANG | Clipboard overlay → "Capital of East Nusa Tenggara" | Confirmed |
| 18 | Commercial | LAHORE | Periodic dots → atomic numbers → LA HO RE | Confirmed |
| 19 | Commercial | LIMA | Belt wire colors 12-7-13-1 | Confirmed |
| 20 | Commercial | TASHKENT | Marked bills → "CASH TENT" → Spoonerism → TASHKENT | Confirmed |
| 21 | Commercial | TALLINN | Subtitles hyphens → "Seven Letter Big Lodge" → tall inn | Confirmed |
| 22 | Commercial | **NEW ORLEANS?** | Tank numbers + ice cream video. Omega <50% confidence. | Unconfirmed |
| 23 | Commercial | **YELLOWSTONE?** | Smoke grenade: YELLOW+S+T+ONE. BeastBot: "color text matters." Not in Omega. | Unconfirmed |
| 24 | Commercial | ??? | Camel/Llama (BeastBot confirmed, "BEASTLAND" = Saudi Arabia) | Unsolved |
| 25 | Commercial | ??? | 12th commercial puzzle — not yet identified | Unsolved |
| 26 | Gift | ALGIERS | Airrack — 7 pictures hide "ERIC." First letters A-L-G-I-E-R-S | Confirmed |
| 27 | Gift | TAMPA | Sketch — Madden 99 → BUCS HOME. Omega ⬤ | Confirmed |
| 28 | Gift | **WICHITA** | Fallon — "JIMMYS BIRTHPLACE" = MrBeast born Wichita. Omega ⬤ | Confirmed |
| 29 | Gift | TIERRA DEL FUEGO | Quackity — Sword + Beagle Channel | Confirmed |
| 30 | Gift | DEKALB | Hayleybaylee — Rings → DeKalb, Illinois | Confirmed |
| 31 | Gift | **MONTREAL** | Jim & Monika — MAD fold-in → MONTREAL CANADA. Omega ⬤ | Confirmed |
| 32 | Gift | CAIRNS, QUEENSLAND | Hemsworth — "LAND OF CAIRNS." Omega ⬤ | Confirmed |
| 33 | Gift | DIVO | McKenna — Card trick → Divo, Côte d'Ivoire | Confirmed |
| 34 | Gift | COPENHAGEN | Pierson — Blender → HAFNIUM CITY | Confirmed |
| 35 | Gift | BUFFALO | Botez — Chess Game 6 cipher | Confirmed |
| 36 | Gift Unifier | ??? | Golden Present Matrix — 10 numbered squares with (x,y) pairs. | Unsolved |
| 37 | YT Short | **QUITO, Ecuador** | Mirror video — KEY-TOW-HECK-QUID-DOOR = "Quito Ecuador" | Confirmed |
| 38 | YT Short | ??? | Chocolate heart — MrBeast + Amaury Gaichon | Unsolved |
| 39 | SF: BTS (A1) | **MALÉ, Maldives?** | Malcolm — "Male Mal dives." Omega ⦿ | Probable |
| 40 | SF: BTS (A2) | **ITURUP** | Submarine Math — grid + subscriber count → ASCII. Omega ⬤ | Confirmed |
| 41 | SF: BTS (A3) | YELLOWKNIFE | Puzzle Maker — BTS video 6:19. Omega ⬤ | Confirmed |
| 42 | SF: BTS (A4) | **THANE, India?** | Elephant 85673 → index into ELEPHANT = THANE. Omega ⦿ | Probable |
| 43 | SF: Physical (B1) | **NAZARÉ** | Tower LED video — "SURF RECORD SITE WITH ARGENT HUE." Omega ⬤ | Confirmed |
| 44 | SF: Physical (B2) | **NOME** | Billboard letters — "e mon" backwards. Omega ⬤ | Confirmed |
| 45 | SF: Physical (B3) | **DIMTU** | Billboard numbers — 04 09 13 20 21 → DIMTU. Omega ⬤ | Confirmed |
| 46 | SF: Site (C1) | ??? | Dot-Dollar / "How?" video — unsolved | Unsolved |
| 47 | SF: Site (C2) | **BABYLON?** | FAQ — "PRATE OR THIS" → babble on → BABYLON. Omega ⦿ | Probable |
| 48 | Lone Shark | HEARD ISLAND | Bluesky homophones | Confirmed |
| 49 | Lone Shark | ILE DE LA POSSESSION | Podcast anagram | Confirmed |
| 50 | Lone Shark | ASCENSION ISLAND | BoardGameGeek acrostic | Confirmed |
| — | Crossword | EYJAFJALLAJOKULL | 16 circled cells anagram (51st location, sets up Stage 2) | Confirmed |

*Note: CASABLANCA (Slack emoji → CA-SA-BL-AN-CA) is confirmed as a location but its category slot is uncertain.*

---

**Feedback?** [discord.gg/KCtjNMJeYv](https://discord.gg/KCtjNMJeYv)
